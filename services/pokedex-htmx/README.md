...
### The browser journey — run this by hand

**`make test` does not run this, and neither does CI.** It needs a
browser and a running stack, and Playwright is deliberately not a
dependency of this service. Nothing will tell you if it breaks; run it
yourself when you change how the page behaves.
...
a wait between clicks, which is why the scripted tests all passed.

The whole stack, no database needed — the API keeps state in memory
when `DATABASE_URL` is unset:

```sh
(cd ../pokedex && PORT=3000 go run .) &
POKEDEX_URL=http://127.0.0.1:3000 PORT=3001 INSECURE_COOKIES=1 npm start &

# from a directory that has playwright installed
