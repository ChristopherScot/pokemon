...
export function url(
  { active, join, team, q }: { active?: string; join?: string; team?: string[]; q?: string },
): string {
  const parts: string[] = []
...
const spriteOf = (all: Pokemon[], name: string): string =>
  all.find((p) => p.name === name)?.sprite ?? ''
...
export function teamSlots(ctx: Ctx): string {
  const slots = Array.from({ length: TEAM_SIZE }, (_, i) => {
    ...
    return `<div class="slot filled">` +
      ...
      `<img src="${esc(spriteOf(ctx.pokemon, name))}" alt="${esc(name)}">` +
...
export function card(mon: Pokemon, ctx: Ctx): string {
  ...
  // A disabled control still has to exist for the grid to look right,
  // so this is NOT the HATEOAS case where the server omits the control:
  // "you already have three" is a state the card must show.
...
export function grid(ctx: Ctx): string {
  return `<main class="grid" id="grid">${ctx.pokemon.map((p) => card(p, ctx)).join('')}` +
